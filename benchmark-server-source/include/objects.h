...
#define PRINT_BYTES(array, size) \
    int ocunt = 0; \
    for (uint8_t *cp = array; ocunt < size; ++cp) { printf("%d: %hhx\n", ocunt++, *cp); }
...
#define SOCKET_ADDR_SIZE (sizeof(in_addr_t) + sizeof(in_port_t)) /** The size of an ip:port combination. */
